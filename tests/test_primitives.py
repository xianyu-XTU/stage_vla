...
from stage_vla.vla_light.planner import PlanDecoder, semantic_plan
...
def test_plan_decoder_shape_and_decode():
    """计划头：指令嵌入 → 计划 logits，argmax 解码成动作名。"""
    decoder = PlanDecoder(instr_dim=128, n_primitives=5, plan_len=5)
    emb = torch.randn(2, 128)
    logits = decoder(emb)
    assert tuple(logits.shape) == (2, 5, 5)
    plans = decoder.decode(emb)
    assert len(plans) == 2
    assert all(len(p) == 5 for p in plans)
    assert all(name in {p.name for p in PRIMITIVES} for name in plans[0])


def test_plan_decoder_trains_to_match_semantic():
    """过拟合一个小批次：让计划头学会把指令嵌入解码成语义计划（验证可训练性）。"""
    torch.manual_seed(0)
    decoder = PlanDecoder(instr_dim=16, n_primitives=5, plan_len=3)
    opt = torch.optim.Adam(decoder.parameters(), lr=1e-2)
    # 目标：approach→grasp→lift（索引 0,1,2）
    emb = torch.randn(8, 16)
    target = torch.tensor([0, 1, 2]).repeat(8, 1)
    for _ in range(200):
        logits = decoder(emb)
        loss = torch.nn.functional.cross_entropy(logits.reshape(-1, 5), target.reshape(-1))
